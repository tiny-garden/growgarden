...
// ==========================================================
// INDIVIDUAL PET INSTANCES
// Every pet obtained from an egg is a separate pet.
// Two Dragonflies = two separate instances with separate
// equip state, cooldowns, XP and skills.
// ==========================================================
const petInstances = [];
let nextPetInstanceId = 1;

function createPetInstance(type, equipped = false) {
    const instance = {
...
function getPetInstances(type, activeOnly = false) {
    return petInstances.filter(p =>
        p.type === type && (!activeOnly || p.equipped)
    );
}

function getActivePetInstances() {
    return petInstances.filter(p => p.equipped);
}

function syncPetCounts() {
    Object.values(petData).forEach(p => {
        p.count = 0;
        p.equipped = 0;
    });

    petInstances.forEach(instance => {
        if (!petData[instance.type]) return;
        petData[instance.type].count++;
        if (instance.equipped) petData[instance.type].equipped++;
    });
}

function migrateOldPetSave(saved) {
    petInstances.length = 0;
    nextPetInstanceId = 1;

    // New save format
    if (Array.isArray(saved.petInstances)) {
        saved.petInstances.forEach(raw => {
            if (!raw || !petData[raw.type]) return;
            const instance = createPetInstance(raw.type, !!raw.equipped);
            instance.instanceId = raw.instanceId || instance.instanceId;
            instance.xp = Number(raw.xp || 0);
            instance.lastSkill = Number(raw.lastSkill || 0);
            instance.prayingUntil = Number(raw.prayingUntil || 0);

            const n = Number(String(instance.instanceId).replace("pet_", ""));
            if (Number.isFinite(n)) nextPetInstanceId = Math.max(nextPetInstanceId, n + 1);
        });
    }

    // Migrate old saves where pets were grouped by type.
    if (!petInstances.length && saved.petCounts) {
        Object.entries(saved.petCounts).forEach(([type, count]) => {
            const total = Math.max(0, Number(count || 0));
            const equipped = Math.min(
                total,
                Number(saved.petEquipped?.[type] || 0)
            );

            for (let i = 0; i < total; i++) {
                createPetInstance(type, i < equipped);
            }
        });
    }
...
function renderPets() {
    ...
    petInstances.forEach((instance, index) => {
        const pet = petData[instance.type];
        if (!pet) return;

        const card = document.createElement("div");
        card.className = "pet-card" + (instance.equipped ? " equipped" : "");

        card.innerHTML = `
            <div class="icon">${pet.emoji}</div>
            <div class="info">
                <b>${pet.name} #${index + 1}</b>
                <small>XP: ${Math.floor(instance.xp || 0)} · ${instance.equipped ? "Equipped" : "Stored"}</small>
            </div>
            <button type="button">
                ${instance.equipped ? "Unequip" : "Equip"}
            </button>
            <div class="skill">Skill: ${pet.skill}</div>
        `;

        const button = card.querySelector("button");
        if (instance.equipped) {
            button.onclick = () => unequipPet(instance.instanceId);
        } else {
            button.disabled = equippedCount() >= 3;
            button.onclick = () => equipPet(instance.instanceId);
        }
...
function renderActiveSlots() {
    const box = document.getElementById("activeSlots");
    if (!box) return;

    box.innerHTML = "";

    const active = getActivePetInstances();

    for (let i = 0; i < 3; i++) {
        const slot = document.createElement("div");
        slot.className = "active-slot" + (active[i] ? " filled" : "");
        ...
        if (active[i]) {
            ...
            slot.innerHTML = `
                <span>${pet.emoji}</span>
                ${pet.name}
                <button class="remove-pet" type="button">✕</button>
            `;

            slot.querySelector(".remove-pet").onclick = () =>
                unequipPet(instance.instanceId);
        } else {
...
function equippedCount() {
    return petInstances.filter(p => p.equipped).length;
}

function getPetInstance(instanceId) {
    return petInstances.find(p => p.instanceId === instanceId);
}

function equipPet(instanceId) {
    const instance = getPetInstance(instanceId);
    if (!instance) return;
    ...
    const pet = petData[instance.type];
    notify(pet.emoji + " " + pet.name + " equipped!");
}

function unequipPet(instanceId) {
    const instance = getPetInstance(instanceId);
    if (!instance || !instance.equipped) return;
    ...
    const pet = petData[instance.type];
    notify(pet.emoji + " " + pet.name + " unequipped!");
}
...
function runForEachActivePet(type, callback) {
    getPetInstances(type, true).forEach(instance => callback(instance, petData[type]));
}

function bunnySkill() {
    runForEachActivePet("bunny", (instance, pet) => {
        if (Date.now() - instance.lastSkill < 30000) return;

        const carrots = plots.filter(plot => plot.crop === "carrot" && isCropReady(plot));
        ...
        instance.lastSkill = Date.now();
        instance.xp += 10;
        updateUI();
        saveGame();
        notify("🐰 " + pet.name + " ate a Carrot → +" + gain + " coins (1.5×)!");
    });
}

function digSeedSkill(petId, chance, label) {
    runForEachActivePet(petId, (instance, pet) => {
        if (Date.now() - instance.lastSkill < 60000) return;

        if (Math.random() < chance) {
            const type = weightedSeedType();
            seeds[type]++;
            notify(pet.emoji + " " + label + " found 1 " + cropTypes[type].name + " Seed!");
            updateUI();
        }
...
function prayingMantisSkill() {
    runForEachActivePet("prayingMantis", (instance, pet) => {
        if (Date.now() - instance.lastSkill < 80000) return;
        ...
        target.mantisOwnerId = instance.instanceId;

        instance.lastSkill = Date.now();
        instance.prayingUntil = Date.now() + 10000;
        instance.xp += 10;

        notify(pet.emoji + " " + pet.name + " is praying for 10 seconds!");
        saveGame();
    });
}

function dragonflySkill() {
    runForEachActivePet("dragonfly", (instance, pet) => {
        if (Date.now() - instance.lastSkill < 150000) return;
        ...
        if (addMutation(plot, "gold")) {
            instance.lastSkill = Date.now();
            instance.xp += 10;
            notify("🐉 " + pet.name + " turned " + cropTypes[plot.crop].name + " Gold!");
            saveGame();
        }
    });
}
...
function hedgehogMultiplier(plot) {
    return getPetInstances("hedgehog", true).length > 0 &&
        cropTypes[plot.crop]?.category === "prickly" ? 1.5 : 1;
}

function moleSkill() {
    runForEachActivePet("mole", (instance, pet) => {
        if (Date.now() - instance.lastSkill < 80000) return;
        ...
        if (Math.random() < 0.7) {
            ...
            coins += gain;
            notify("🐹 " + pet.name + " dug up " + gain + " coins!");
        } else {
            const gearId = weightedGearType();
            gearInventory[gearId] = (gearInventory[gearId] || 0) + 1;
            notify("🐹 " + pet.name + " found " + gearData[gearId].emoji + " " + gearData[gearId].name + "!");
        }
...
function skipGrowthByMinutes(petId, minutes, label, cooldown) {
    runForEachActivePet(petId, (instance, pet) => {
        if (Date.now() - instance.lastSkill < cooldown) return;
        ...
        const currentGrowth = getGrowth(plot);

        // Never skip beyond fully grown.
        const remainingMs = Math.max(0, (1 - currentGrowth) * getCropGrowTime(plot) / Math.max(speed, 0.0001));
        const skipMs = Math.min(minutes * 60 * 1000, remainingMs);

        plot.plantedAt -= skipMs / speed;

        instance.lastSkill = Date.now();
        instance.xp += 10;
        notify(pet.emoji + " " + label + " skipped 30 minutes of growth!");
        saveGame();
    });
}
...
function raccoonSkill() {
    runForEachActivePet("raccoon", (instance, pet) => {
        if (Date.now() - instance.lastSkill < 600000) return;
        ...
        notify("🦝 " + pet.name + " duplicated " + cropTypes[plot.crop].name + " → Harvest Bag!");
    });
}

function giantAntDuplicate(item) {
    if (!getPetInstances("giantAnt", true).length) return false;
    return Math.random() < 0.10;
}

function snailSeed() {
    if (!getPetInstances("snail", true).length) return false;
    return Math.random() < 0.05;
}

function getGrowthSpeedMultiplier(plot) {
    let multiplier = 1;
    if (getPetInstances("caterpillar", true).length > 0 &&
        plot.crop &&
        cropTypes[plot.crop]?.category === "leafy") {
        multiplier *= 2;
    }
    return multiplier;
}
...
function renderHarvestBag() {
    ...
    const count = document.getElementById("harvestCount");
    const selectedCount = document.getElementById("selectedHarvestCount");

    if (!box) return;

    // Remove selections for items that no longer exist.
    const validIds = new Set(harvestBag.map(item => item.id));
    selectedHarvestItems.forEach(id => {
        if (!validIds.has(id)) selectedHarvestItems.delete(id);
    });

    box.innerHTML = "";

    if (count) count.textContent = harvestBag.length;
    if (selectedCount) selectedCount.textContent = selectedHarvestItems.size;

    const sellSelected = document.getElementById("sellSelectedButton");
    const sellAll = document.getElementById("sellAllButton");

    if (sellSelected) sellSelected.disabled = selectedHarvestItems.size === 0;
    if (sellAll) sellAll.disabled = harvestBag.length === 0;
    ...
    harvestBag.forEach(item => {
        const crop = cropTypes[item.type];
        if (!crop) return;

        const element = document.createElement("button");
        element.type = "button";
        element.className = "harvest-item";
        element.setAttribute("aria-pressed", selectedHarvestItems.has(item.id) ? "true" : "false");

        if (selectedHarvestItems.has(item.id)) {
            element.classList.add("selected");
        }

        const tier = getWeightTier(item.weight, crop.minWeight);
        const mutations = Array.isArray(item.mutations)
            ? item.mutations
            : (Array.isArray(item.mutation)
                ? item.mutation
                : (item.mutation ? [item.mutation] : []));

        const price = Math.floor(getHarvestPrice(item));

        element.innerHTML = `
            <span class="harvest-select-mark">
                ${selectedHarvestItems.has(item.id) ? "✓" : ""}
            </span>
            <span class="harvest-icon">${crop.emoji}</span>
            <span class="harvest-name">${mutationEmojis(mutations)} ${crop.name}</span>
            <span class="harvest-mutation">${mutationLabel(mutations)}</span>
            <span class="harvest-weight">${Number(item.weight || 0).toFixed(1)} kg</span>
            <span class="harvest-tier">${tier.emoji} ${tier.name}</span>
            <span class="harvest-price">💰 ${price.toLocaleString()}</span>
        `;
...
function saveGame() {
    ...
    const saveData = {
        ...
        petInstances: petInstances.map(instance => ({ ...instance })),
...
const sellSelectedButton = document.getElementById("sellSelectedButton");
const sellAllButton = document.getElementById("sellAllButton");

if (sellSelectedButton) sellSelectedButton.addEventListener("click", event => {
    event.preventDefault();
    event.stopPropagation();
    sellSelectedCrops();
});

if (sellAllButton) sellAllButton.addEventListener("click", event => {
    event.preventDefault();
    event.stopPropagation();
    sellAllCrops();
});
...
function drawPets() {
    const activePets = getActivePetInstances();

    activePets.forEach((instance) => {
        const pet = petData[instance.type];
        const uniqueKey = instance.instanceId;
        ...
        if (!petRuntime[uniqueKey]) {
            petRuntime[uniqueKey] = {
                ...
                targetY: 90 + Math.random() * 400
            };
        }

        const runtime = petRuntime[uniqueKey];

        if (Math.hypot(runtime.targetX - runtime.x, runtime.targetY - runtime.y) < 8) {
            runtime.targetX = 100 + Math.random() * 700;
            runtime.targetY = 90 + Math.random() * 400;
        }
        ...
        runtime.x += dx / distance * 0.8;
        runtime.y += dy / distance * 0.8;

        ctx.font = "35px Arial";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(pet.emoji, runtime.x, runtime.y);
    });
}
...
function updateNightOwlXP() {
    const owls = getPetInstances("nightOwl", true);
    if (!owls.length) return;

    getActivePetInstances().forEach(instance => {
        if (instance.type === "nightOwl") return;
        instance.xp += 30 / 60;
    });
}
